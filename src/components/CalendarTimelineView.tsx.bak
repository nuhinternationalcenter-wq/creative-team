...
export const CalendarTimelineView: React.FC<CalendarTimelineViewProps> = ({
  ...
  // Modals for instant handover from calendar
      ...
      >
        <div className="flex items-start space-x-3 flex-1">
          ...
          <div className="space-y-1">
            ...
            <h4
              onClick={() => {
                if (item.originalStep) onOpenStepDetail(item.originalStep);
              }}
                  ...
                  <div className="space-y-1 overflow-hidden flex-1">
                    {dayTasks.slice(0, 3).map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`text-[10px] px-1.5 py-0.5 rounded truncate font-medium flex items-center space-x-1 ${
                            item.isCompleted
                          ...
                          title={`${item.title} (${item.assignedTo})`}
                        >
