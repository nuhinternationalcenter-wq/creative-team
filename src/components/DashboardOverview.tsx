...
export const DashboardOverview: React.FC<DashboardOverviewProps> = ({
  ...
  // Team Chain Metrics
  const totalChainSteps = activeProject ? activeProject.steps.length : 0;
  const completedChainSteps = activeProject ? activeProject.steps.filter((s) => s.status === 'completed').length : 0;
  const inProgressChainSteps = activeProject ? activeProject.steps.filter((s) => s.status === 'in_progress') : [];
  const waitingChainSteps = activeProject ? activeProject.steps.filter((s) => s.status === 'waiting_approval') : [];
  ...
  const overdueChainSteps = activeProject ? activeProject.steps.filter((s) => s.status !== 'completed' && s.dueDate < todayStr) : [];
